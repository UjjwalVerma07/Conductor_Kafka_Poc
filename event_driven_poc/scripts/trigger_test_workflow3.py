#!/usr/bin/env python3
"""
Trigger Test Workflow 3 (test_workflow3)
Simple script to trigger the test_workflow3
Automatically registers the workflow if not already registered
Service order: Airflow → Email → Phone → Enrichment
"""
...
WORKFLOW_NAME = "test_workflow3"
WORKFLOW_FILE = os.path.join(os.path.dirname(__file__), "..", "workflows", "test_workflow3.json")
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Trigger Test Workflow 3')
    parser.add_argument('--update', action='store_true',
                       help='Force re-register/update the workflow even if already registered')
    args = parser.parse_args()
    
    print("=" * 60)
    print(f"Test Workflow 3 Manager (test_workflow3)")
    print("Service Order: Airflow → Email → Phone → Enrichment")
    print("=" * 60)
    ...
    print(f"   - Service execution order: Airflow → Email → Phone → Enrichment")
    print()
