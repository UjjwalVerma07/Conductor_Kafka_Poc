...
class IngestionService:
    ...
    1. Only download/upload input file to S3 if it's coming from MinIO
    2. If input is already S3 path, skip download & upload
    3. Fetch JSON from S3
    4. Update input/output/report URIs
    5. Re-upload updated JSON to S3
    """

        # Generate run ID if not provided
        # Run id is basically the unique id for each run which is generated by timestamp and uuid
        ...
        # Step 1: Download file from MinIO to local temp if input_uri is MINIO
        ...
        if input_uri.startswith("minio://"):
            ...
            # Already S3 path , no re-upload needed
        ...
        #For poc purpose the source json s3 key and target s3 key location are same
...
if __name__=="__main__":
    ...
    minio_uri = "minio://raw-data/1000861642.in"
    workflow_id = "workflow-test-123"

    results, final_uri = service.execute_service_chain(
        workflow_json=workflow_json,
        minio_input_uri=minio_uri,
        workflow_id=workflow_id
    )
