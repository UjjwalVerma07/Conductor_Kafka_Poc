#!/usr/bin/env python3
"""
Scalable Conductor Event Router Service
Consumes completion events from Kafka and completes EVENT tasks via API

This router provides scalable event-driven architecture:
- Services publish to Kafka only (no API calls - scales to 100-150 services)
- Router consumes from Kafka and completes EVENT tasks (centralized, scalable)
- Router can be scaled horizontally for high throughput
"""
...
class ScalableEventRouter:
    ...
    def find_matching_event_task(self, workflow_id, event_type, task_id_from_event=None, max_retries=5, retry_delay=2):
        """
        Dynamically find EVENT task that matches the event type by:
        1. Using taskId from event if provided (most direct)
        2. Matching event type to EVENT task's sink property
        3. Fallback to hardcoded mapping if needed
        
        Returns: (task_id, task_ref_name, task_status) or (None, None, None)
        """
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(f"⚠️ No matching EVENT task found for event type '{event_type}' after {max_retries} attempts")
    ...
    def process_completion_event(self, event):
        """
        Process completion event and complete corresponding EVENT task dynamically.
        No hardcoded mappings required - discovers EVENT tasks from workflow.
        """
        try:
            ...
            if not task_id:
                logger.warning(f"⚠️ No matching EVENT task found for event type '{event_type}' in workflow {workflow_id}")
                return
            
            if task_status != 'IN_PROGRESS':
                logger.warning(f"⚠️ EVENT task '{task_ref_name}' status is {task_status}, not IN_PROGRESS. Skipping.")
                return
    ...
    def consume_and_complete(self):
        ...
        logger.info("🚀 Scalable Event Router started (Dynamic Mode)")
        logger.info(f"📡 Listening to Kafka topic: conductor-events")
        logger.info(f"🔗 Conductor API: {CONDUCTOR_API}")
        logger.info(f"💡 Dynamic EVENT task discovery enabled:")
        logger.info(f"   - Matches events to EVENT tasks by sink property")
        logger.info(f"   - Uses taskId from event if available")
        logger.info(f"   - Fallback to hardcoded mapping if needed ({len(self.event_to_task_map)} mappings)")
        logger.info(f"💡 This router enables scalable architecture:")
        logger.info(f"   - Services publish to Kafka only (no API calls)")
        logger.info(f"   - Router dynamically discovers and completes EVENT tasks")
        logger.info(f"   - Router can be scaled horizontally for high throughput")
        logger.info(f"   - No hardcoded task names required - works with any EVENT task name!")
